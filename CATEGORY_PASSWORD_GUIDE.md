...
访客输入正确密码后，可以访问相应的加密内容。浏览器保存的是使用 Typecho 站点密钥签名、有效期为 7 天的 HMAC 解锁票据，不是密码或可永久重放的裸哈希。单篇文章、分类和全站密码使用不同作用域的 Cookie，解锁一种内容不会覆盖另一种内容的状态。
...
- 票据与当前所需密码绑定；单篇文章 cid、分类 slug、全站密码和内联内容块使用隔离的 Cookie 名称，中文 slug 也不会相互冲突
- 页面和内联密码表单带有绑定当前页面上下文的匿名双提交 CSRF 令牌；缺少主题签发 Cookie 的直接 POST 会被拒绝
